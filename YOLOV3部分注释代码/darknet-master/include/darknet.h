...
struct layer{
    ...
    int nweights;    // 有多少个权重值 c/groups*n*size*size;其中 groups=1
    int nbiases;     // 有多少个偏置
    int extra;
    int truths;      // 根据yolo_layer.c判断，这个变量表示一张图片含有的真实值的个数，对于检测模型来说，一个真实的标签含有5个值，
                     // 包括类型对应的编号以及定位矩形框用到的w,h,x,y四个参数，且在darknet中，固定每张图片最大处理90个矩形框（可查看max_boxes参数），
                     // 因此，在yolo_layer.c的make_yolo_layer()函数中，赋值为30*5
    ...
    */
    ...
    float * spatial_mean;   // 这些属性对应BN的参数
    float * mean;
    float * variance;
    ...
    size_t workspace_size;  // net.workspace的元素个数，为所有层中最大的l.out_h*l.out_w*l.size*l.size*l.c（在make_convolutional_layer()
	                        // 计算得到workspace_size的大小，在parse_network_cfg()中动态分配内存，此值对应未使用gpu时的情况）


#ifdef GPU
    int *indexes_gpu;       // CPU使用参数（GPU中按名字对应也有一套）
...
typedef struct network{
    int n;                                // 网络总层数（make_network()时赋值），不包括cfg中[net]层，即除了[net]外所有[xxx]的总个数
    ...
    int *t;
    float epoch;                          // 训练多少个epoch便结束训练
    int subdivisions;                     // 具体含义未知。
    layer *layers;                        // 存储网络所有的层，在make_network()中动态分配内存
    float *output;                        // 存储网络的输出？
    learning_rate_policy policy;          // 学习率调整策略 learning_rate_policy是定义在本文件的一种枚举类型

    float learning_rate;                  // 学习率大小，一般在cfg文件中指定
    float momentum;                       // 学习率动量
    float decay;                          // 学习率衰减正则项，防止过拟合
    float gamma;
    float scale;
    float power;
    int time_steps;
    int step;
    int max_batches;                      // 网络训练最多的batch数
    float *scales;
    int   *steps;
    int num_steps;                       // 对应steps中调整学习率的次数
    int burn_in;
    ...
    int gpu_index;                      // 网络所使用的GPU编号
    tree *hierarchy;

    float *input;                       // 中间变量，用来暂存某层网络的输入（包含一个batch的输入，比如某层网络完成前向，将其输出赋给该变量，
	                                    // 作为下一层的输入，可以参看network.c中的forward_network()与backward_network()两个函数），
                                        // 当然，也是网络接受最原始输入数据（即第一层网络接收的输入）的变量（比如在图像检测训练中，
										// 最早在train_detector()->train_network()->get_next_batch()函数中赋值）
    float *truth;                       // 中间变量，与上面的input对应，用来暂存input数据对应的标签数据（真实数据）
    float *delta;                       // 中间变量，用来暂存某层网络的敏感度图（反向传播处理当前层时，用来存储上一层的敏感度图，因为当前层会
	                                    // 计算部分上一层的敏感度图，可以参看network.c中的backward_network()函数），
                                        // net.delta并没有在创建网络之初就为其动态分配了内存，而是等到反向传播时，直接将其等于某一层的l.delta
										//（l.delta是在创建每一层网络之初就动态为其分配了内存），这才为net.delta分配了内存，
    ...
    int train;                          // 标志参数，网络是否处于训练阶段，如果是，则值为1（这个参数一般用于训练与测试有不同操作的情况，
	                                    // 比如dropout层，对于训练，才需要进行forward_dropout_layer()函数，对于测试，不需要进入到该函数）
