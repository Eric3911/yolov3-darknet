...
/****************************************************************
*  func：将filenames中的内容按行组织成节点插入双向链表中        *
*  args filename：欲处理的文件全称，如："data/coco.names"       *
*  details：返回双向链表的头指针,具体注释与read_data_cfg类似    *
****************************************************************/
list *get_paths(char *filename)
{
	...
	// 按行读取文件类容，直接作为node的值域val插入链表lines
...
/********************************************************************************
*  func: 从paths中读取n条路径信息,返回一个二维数组（矩阵），每行代表一张图片的  *
*       路径，共n行                                                             *
*  args paths: paths包含所有训练图片的路径，二维数组，每行对应一张图片的路径    *
*  args n: 要读取的条数                                                         *
*  args m：为paths的行数，即为训练图片总数                                      *
********************************************************************************/
char **get_random_paths(char **paths, int n, int m)
{
	// 需要读取n张图片，开辟一个n维数组保存图片路径
    char **random_paths = calloc(n, sizeof(char*));
    int i;
	// paths这个变量可能会被不同线程访问（读取数据本来就是多线程的），所以访问之前，先锁住，结束后解锁
	// 当pthread_mutex_lock()返回时，该互斥锁已被锁定。线程调用该函数让互斥锁上锁，如果该互斥
	// 锁已被另一个线程锁定和拥有，则调用该线程将阻塞，直到该互斥锁变为可用为止。
	// 函数原型：int pthread_mutex_lock(pthread_mutex_t *mutex);
    pthread_mutex_lock(&mutex);
    for(i = 0; i < n; ++i){
		// 随机产生索引：随机读入图片路径，产生的随机数对m求余保证索引小于m
		// 随意读取图片的目的：举个例子：一般的训练集都是猫的图片在一起，狗的图片在一起，
		// 如果不随机读取，就是一个或者几个batch都是猫或者狗，容易过拟合同时泛化能力也差
        int index = rand()%m;
		// 将随机索引加入到刚才开辟的数组之内
...
/********************************************************************************************************************
*  func: 读入一张图片的所有box,一张图片可能有多个物体，每个物体都有一个矩形框框起来（物体检测不单识别类别，更包括   *
*        定位），本函数就是读入一张图片的所有box信息。每个box包括5条信息，依次为：物体类别id，矩形中心点x坐标，矩   *
*        形中心点y坐标，矩形框宽度w,矩形框高度h。                                                                   *
*  args filename：标签数据所在路径（标签数据需要下载，然后调用voc_label.py生成指定的格式，具体路径视情况而定，相见  *
*                 darknet/yolo网页）                                                                                *
*  args n: 该图片中的物体个数，也就是读到的矩形框个数（也是一个返回值）                                             *
********************************************************************************************************************/
box_label *read_boxes(char *filename, int *n)
{
    ...
    FILE *file = fopen(filename, "r");
    if(!file) file_error(filename);
	// x，y表示BBox中心点坐标，w,h表示BBox的width和height
    float x, y, h, w;
    int id;
    int count = 0;
    int size = 64;
	// 新建64标签数据box，并动态分配内存（如果框的个数多余64，则利用realloc函数重新分配内存）
...
/*************************************************************************************************************
*  func: 矫正矩形框标签数据在标准化尺寸图片中的值：输入的图片，经过place_image()函数对图片尺寸进行规范化以及 *
*        数据增强后,尺寸发生了变化，由于矩形框的x,y,w,h（分别为矩形中心点坐标，矩形宽高）都是相对于原始图片  *
*        宽高的比例值，所以，如果仅对原始图片进行缩放（等比例也好，不等比例也好），是不会改变x,y,w,h等值的， *
*        也就是中间图与原始图矩形框的x,y,w,h值是一样的，但关键在于，在函数place_image()中，不单涉及缩放操作，*
*        还涉及平移操作，place_image()函数的最后一步是将中间图随机的嵌入到最终输出图上，因为这一步，就彻底改 *
*        变了x,y,w,h的值，为了计算新的x,y,w,h值，很简单，先按原来的值乘以中间图的宽高，得到矩形框四个角点的  *
*        真实坐标，而后进行平移，统一除以最终输出图的宽高，得到新的x,y,w,h值。除此之外，左右翻转也会导致     *
*        x,y,w,h值的变化。                                                                                   *
*  args boxes: 一张图片中包含的所有矩形框标签数据                                                            *
*  args n: 一张图片中包含的矩形框个数                                                                        *
*  args dx: place_image()函数中，中间图相对于最终输出图canvas的起点的x坐标，用占比表示（或者说x方向上的偏移  *
*           坐标），正值表示中间图嵌入到最终输出图中，负值表示输出图是中间图的一个mask                       *
*  args dy: place_image()函数中，中间图相对于最终输出图canvas的起点的y坐标，用占比表示（或者说y方向上的偏移  *
*           坐标），正值表示中间图嵌入到最终输出图中，负值表示输出图是中间图的一个mask                       *
*  args sx: nw/w，place_image()函数中中间图宽度与最终输出图宽度的比值                                        *
*  args sy: nw/w，place_image()函数中中间图高度与最终输出图高度的比值                                        *
*  args flip: 是否进行了翻转操作，在load_data_detection()中，为了进行数据增强，还可能进行了翻转操作          *
*************************************************************************************************************/
...
/************************************************************************************************************************************
*  func: 用来获取一张图的真实标签信息，对于图像检测，标签信息包括物体的类别（用类别id表示）以及定位信息，定位用矩形框来表示，包含   *
*        矩形中心点坐标x,y以及宽高w,h，本函数读入一张图片中所有标签信息（一张图片可能存在多个物体，每个物体都含有一条类别信息以及   *
*        一个矩形框信息）                                                                                                           *
*  args path: 一张图片所在路径，字符数组                                                                                            *
*  args num_boxes: 每张图片允许处理的最大的矩形框数（如果图片中包含的矩形框大于num_boxes，那么不管，随机取其中num_boxes个参与训练） *
*  args truth: 存储一张图片包含的所有真实信息（标签信息），相当于返回值，对于检测而言，主要包括物体类别以及定位(矩形框)信息，truth  *
*              是一个一维数组，每张矩形框有5条信息，因此truth中每5个数对应一个矩形框数据                                            *
*  args classes：本函数并未使用该参数                                                                                               *
*  args flip: 图片在之前读入时（比如在load_data_detection函数中）是否进行过了左右翻转                                               *
*  args dx: 此参数需要参考load_data_detection函数中的注释，dx是中间图相对最终图的起点位置的x坐标除以最终图的宽度（并取负值）        *
*  args dy: 此参数需要参考load_data_detection函数中的注释，dy是中间图相对最终图的起点位置的x坐标除以最终图的高度（并取负值）        *
*  args sx: 此参数需要参考load_data_detection函数中的注释，sx是中间图宽度与最终图宽度的比值                                         *
*  args sy: 此参数需要参考load_data_detection函数中的注释，sy是中间图高度与最终图高度的比值                                         *
*  details：后面五个参数，用来矫正矩形框的信息，因为在此函数之前，对输入图片进行了缩放、平移、左右翻转一系列的数据增强操作，这些操  *
*           作不会改变物体的类别信息，但会改变物体的位置信息，也即矩形框信息，需要进行矫正，这些参数的具体含义上面可能未说清，具体  *
*           可参看本函数内部调用的correct_boxes()函数的用法                                                                         *
************************************************************************************************************************************/
void fill_truth_detection(char *path, int num_boxes, float *truth, int classes, int flip, float dx, float dy, float sx, float sy)
{
    ...
    // 比如，图片的路径为：/home/xgh/Darknet_data/VOCdevkit/VOC2007/JPEGImages/000001.jpg，
    // 通过连续调用find_place()函数，最终可以得到对应的标签数据路径labelpath为：
    // /home/xgh/Darknet_data/VOCdevkit/VOC2007/labels/000001.txt
	...
	// find_replace函数：src/utils.c 再path中查找"images"并替换为"labels"再赋值给labelpath，如果没找到，则直接赋值
	// 此处这么处理可能不太适合，要是有其它格式的图片或者文件夹路径不一，就会报错，最好根据需要读者自己修改代码或者读者按作者
	// 对数据的组织方式进行组织
    find_replace(path, "images", "labels", labelpath);
    find_replace(labelpath, "JPEGImages", "labels", labelpath);
    ...
    find_replace(labelpath, ".JPEG", ".txt", labelpath);
    int count = 0;
	// box_label数据结构定义在include/darknet.h 
	/*
	typedef struct{
		int id;
		float x,y,w,h;
	...
	*/
	// read_boxes函数：本文件  读入一张图片的所有box标签信息，count为读到的box个数
    box_label *boxes = read_boxes(labelpath, &count);
	// randomize_boxes函数，本文件  随机打乱一张照片中所有box的索引编号
    randomize_boxes(boxes, count);
	// correct_boxes函数：本文件 从输入的原始图片，到真正给神经网络用的图片，可能经过了平移，随机截取，
	// 左右翻转等数据增强操作，这些操作，都会改变矩形框的值，需要进行矫正
    correct_boxes(boxes, count, dx, dy, sx, sy, flip);
    ...
    for (i = 0; i < count; ++i) {
		...
		// 最后将矩形框信息赋给truth，减去sub主要是前面有剔除一定的框
		// 注意标签的存储格式：每一行存储一张图片的边框信息，按段存储5个信息，即每一行的存储方式为：
		// xywh ID xywh ID sywhID....每一个框的信息连续存放
        truth[(i-sub)*5+0] = x;
        truth[(i-sub)*5+1] = y;
        truth[(i-sub)*5+2] = w;
        truth[(i-sub)*5+3] = h;
...
/******************************************************************
*  func：首先调用get_paths()从filename中读取数据到list变量中，而  *
*        后调用list_to_array()转存至二维字符数组中返回            *
*  args filename: 存放标签的文件路径                              *
*  details：从文件中读取数据集中所有类别的名称/标签信息           *
******************************************************************/
char **get_labels(char *filename)
{
	// get_paths函数：本文件。将filename所指向文件的内容读取并按行生成
	// 节点插入双向链表plist中
    list *plist = get_paths(filename);
	// list_to_array函数：src/list.c 将plist的节点的val提取到数组labels
	...
	// free_list函数：src/list.c 释放plist所指链表的所有节点
    free_list(plist);
    return labels;
}
...
/***********************************************************************************************************
*  func: 释放数据d的堆内存                                                                                 *
*  details：该函数输入虽然是data类型数据，但实际上是释放data结构体中matrix类型元素X,y的vals的堆内存（有两  *
*           层深度），分两种情况，浅层释放与深层释放，决定于d的标志位shallow的取值，shallow=1表示浅层释放，*
*           shallow=0表示深层释放，关于浅层与深层释放，见data结构体定义处的注释，此不再赘述。什么时候需浅  *
*           层释放呢？当不想删除二维数组的数据，只想清除第一层指针存储的第二层指针变量时，采用浅层释放，在 *
*           释放前，二维数组的数据应当转移，使得另有指针可以访问这些保留的数据；而当不想要保留任何数据时， *
*           采用深层释放。                                                                                 *
***********************************************************************************************************/
...
/*****************************************************************************************************************
*  func: 从所有训练图片中，随机读取n张，并对这n张图片进行数据增强，同时矫正增强后的数据标签信息。最终得到的图片  *
*        的宽高为w,h（原始训练集中的图片尺寸不定），也就是网络能够处理的图片尺寸，数据增强包括：对原始图片进行宽 *
*        高方向上的插值缩放（两方向上缩放系数不一定相同），下面称之为缩放抖动；随机抠取或者平移图片（位置抖动）；*
*        在hsv颜色空间增加噪声（颜色抖动）；左右水平翻转，不含旋转抖动。可以参考对图像进行jitter处理的各种效果:  *
*        https://medium.com/@vivek.yadav/dealing-with-unbalanced-data-generating-additional-data-by-jittering-   *
*        the-original-image-7497fe2119c3                                                                         *
*  args n：一个线程读入的图片张数（详见函数内部注释）                                                            *
*  args paths: 所有训练图片所在路径集合，是一个二维数组，每一行对应一张图片的路径（将在其中随机取n个）           *
*  args m：paths的行数，也即训练图片总数                                                                         *
*  args w: 网络能够处理的图的宽度（也就是输入图片经过一系列数据增强、变换之后最终输入到网络的图的宽度）          *
*  args h: 网络能够处理的图的高度（也就是输入图片经过一系列数据增强、变换之后最终输入到网络的图的高度）          *
*  args boxes: 每张训练图片最大处理的矩形框数（图片内可能含有更多的物体，即更多的矩形框，那么就在其中随机选择    *
*              boxes个参与训练，具体执行在fill_truth_detection()函数中）                                         *
*  args classes: 类别总数，本函数并未用到（fill_truth_detection函数其实并没有用这个参数）                        *
*  args jitter: 这个参数为缩放抖动系数，就是图片缩放抖动的剧烈程度，越大，允许的抖动范围越大（所谓缩放抖动，就   *
*               是在宽高上插值缩放图片，宽高两方向上缩放的系数不一定相同）                                       *
*  args hue: 颜色（hsv颜色空间）数据增强参数：色调（取值0度到360度）偏差最大值，实际色调偏差为-hue~hue之间随机值 *
*  args saturation: 颜色（hsv颜色空间）数据增强参数：色彩饱和度（取值范围0~1）缩放最大值，实际为范围内的随机值   *
*  args exposure: 颜色（hsv颜色空间）数据增强参数：明度（色彩明亮程度，0~1）缩放最大值，实际为范围内的随机值     *
*  details: 返回data类型数据，包含一个线程读入的所有图片数据（含有n张图片）。最后四个参数用于数据增强，主要对原  *
*           图进行缩放抖动，位置抖动（平移）以及颜色抖动（颜色值增加一定噪声），抖动一定程度上可以理解成对图像   *
*           增加噪声。通过对原始图像进行抖动，实现数据增强。最后三个参数具体用法参考本函数内调用的               *
*           random_distort_image()函数。从此函数可以看出，darknet对训练集中图片的尺寸没有要求，可以是任意尺寸的  *
*           图片，因为经该函数处理（缩放/裁剪）之后，不管是什么尺寸的照片，都会统一为网络训练使用的尺寸          *
*****************************************************************************************************************/
data load_data_detection(int n, char **paths, int m, int w, int h, int boxes, int classes, float jitter, float hue, float saturation, float exposure)
{
	// paths包含所有训练图片的路径
	// get_random_paths函数：本文件 从paths中随机提出n条，即为此次读入的n张图片的路
    char **random_paths = get_random_paths(paths, n, m);
    int i;
	// 初始化为0,清楚内存中之前的旧值（类似data d ={};，参考：https://www.zhihu.com/question/46405621/answer/101218929）
	/* data的数据结构定义如下，在include/darknet.h中
		typedef struct{
			int w, h;
			matrix X;
			matrix y;
	...
	*/
	...
	// d.X中每行就是一张图片的数据，因此d.X.cols等于h*w*3，一个线程读多少张图片就有多少行，所以d.X.rows=n
	...
	// make_matrix函数：src/matrix.c 构造一个n行，5*boxes列的矩阵，其val的值全为0,d.y的每一行表示一张图片的标签，所以一共有n行
    d.y = make_matrix(n, 5*boxes);
	
	// 依次读入每一张图片到d.X.vals的适当位置，同时读入对应的标签信息到d.y.vals的适当位置
    for(i = 0; i < n; ++i){
		// load_image_color函数：src/image.c 读入原始图像，返回image类型的图像结构体，image定义在include/darknet.h中
		// load_image整个过程比较复杂，均已详细注释，可以细读 现在orig中保存了图片的像素数据和w,h,c信息
        image orig = load_image_color(random_paths[i], 0, 0);
		// sized为原始图片经过一系列处理（重排及变换）之后的最终得到的图片，并初始化像素值全为0.5（下面会称之为输出图或者最终图之类的）
		// make_image函数：src/image.c  调用make_empty_image函数，生成w,h,c的空白图片并返回，sized图像与原图像通道保持一致
        image sized = make_image(w, h, orig.c);
		// fill_image函数：src/image.c  将sized图像的所有像素置为0.5
        fill_image(sized, .5);

		// 缩放抖动大小：缩放抖动系数乘以原始图宽高即得像素单位意义上的缩放抖动，以yolov3.cfg中yolo层的jitter=0.3为例，假设输入图像的
		// 尺寸为：1280*720，则dw=1280*0.3=384,dh=720*0.3=216,由此 new_ar的值范围为：（1280-384）/（720+216）-（1280+384）/（720-216）之间
		// 即  0.95 ≤ new_ar ≤ 3.3 这样先生成new_ar的值，固定中间图像的宽高比，再确定中间图像各自的宽高，由此观察jitter设置这么大，
		// 是否会对训练效果产生影响，因为1280*720的图片的宽高比为1.77左右，而使用了jitter=0.3后，最大的宽高比编程了3.3，几乎是原先的2倍
		// 假如现在要检测的是人，你把一个人高度不变，宽度变为了原来的2倍，这个人还是一个很好的样本吗？有实验过的老铁请私密我哈，在此，
		// 本人建议使用jitter的值为0.2，这样的范围可能更合理，你觉得呢？
        float dw = jitter * orig.w;
        float dh = jitter * orig.h;

		// 中间图的长宽比（aspect ratio，此处其实是宽高比）：新的长宽比是一个随机数
		// rand_uniform函数：src/utils.c 生成一个服从-dw到dw之间均匀分布的数
		...
		// 此处scale设置为1表示中间图以长边为标准进行缩放，即w和H中较长的边保持与网络的w和h一致，另一条边按照new_ar进行缩放
        float scale = 1;

		// nw,nh为中间图的宽高, new_ar为中间图的宽高比,由于加入了随机抖动，所以新图像的宽高比不一定等于原图像的宽高比
        float nw, nh;
		...
		// 此处dx和dy应理解为中间图像相对于sized在水平和垂直方向上取值的偏移，具体见函数应用处
        float dx = rand_uniform(0, w - nw);
		// 得到0~h-nh之间的均匀随机数（h-nh可能大于0,可能小于0）
        float dy = rand_uniform(0, h - nh);

		// place_image函数：src/image.c 先用双线性插值对输入图像im进行重排得到一个虚拟中间图
		// 根据中间图的尺寸nw,nh进行重排（双线性插值，不是等比例缩放，长宽比可能会变），而后，将中间图放入到sized，
        // dx,dy是将中间图放入到sized的起始坐标位置（dx,dy若大于0,说明sized的尺寸大于中间图的尺寸，这时
        // 可以说是将中间图随机嵌入到sized中的某个位置；dx,dy若小于0,说明sized的尺寸小于中间图的尺寸，这时
		// sized相当于是中间图的一个mask，在中间图上随机抠图），实质上dx和dy的值在这儿是不会小于0的，因为scale=1
	    // 在此，作图举例进行说明：
		
		//   @@@@@@@@@@@@@@@@@@@@@@@@@                                         @@@@@@@@@@@@@@@@@@
		//   @                       @                                         @                @
		//   @      ########         @                                         @   ######       @
		//   @      # 原图 #         @ 随机改变宽高比，其中一条长边以网络      @   ######       @
		//   @      ########         @ 输入为标准，另外一条边按宽高比得到      @   中间图       @
		//   @                       @                                         @                @
		//   @                       @                                         @@@@@@@@@@@@@@@@@@
		//   @@@@@@@@@@@@@@@@@@@@@@@@@
		
		// 需要注意的一点是，中间图的像素点来源于原图，但中间图的尺寸（实质上）依赖于随机生成的宽高比和网络的输入width和height，
		// 中间图具体得到的步骤为：首先在jitter的抖动范围内随机生成宽高比，但只有宽和高的比值还不能确定宽和高的具体值，得确定其中
		// 的一条边，再按照宽高比算出另外一条边，怎么确定其中的一条边呢？还得根据中间图的宽高比来。如果宽高比大于1，则说明宽度
		// 大于高度，则中间图的宽等于网络输入的width，高按照宽高比计算，相反宽高比小于1时不言而喻，总之一点就是中间图的其中一条边
		// 的大小是等于网络输入尺寸大小的其中一维的
        place_image(orig, nw, nh, dx, dy, sized);

		// random_distort_image函数：src/image.c 随机对图像jitter（在hsv三个通道上添加扰动），实现数据增强
        random_distort_image(sized, hue, saturation, exposure);
		
		// 随机的决定是否进行左右翻转操作来实现数据增强（注意是直接对sized，不是对原始图，也不是中间图）
        int flip = rand()%2;
		// flip_image函数：src/image.c 左右翻转图片sized（本地翻转，即sized也是输出）
        if(flip) flip_image(sized);
		...
		// fill_truth_detection函数：本文件 用来获取一张图的真实标签信息
        fill_truth_detection(random_paths[i], boxes, d.y.vals[i], classes, flip, -dx/w, -dy/h, nw/w, nh/h);

		// free_image函数：src/image.c 释放结构体image
        free_image(orig);
    }
	// 一个小批次的数据读取完毕后，释放动态申请的空间
    free(random_paths);
    return d;
}
...
/********************************************************************
*  func：根据args.type的不同调用不同的函数                          *
*  args ptr：包含该线程要读入图片数据的信息（读入多少张，读入图片最 *
*            终的宽高，图片路径等等）                               *
********************************************************************/
...
/***********************************************************************************
*  func：创建一个线程，读入相应图片数据（此时args.n不再是一次迭代读入的所有图片的  *
*        张数，而是经过load_threads()均匀分配给每个线程的图片张数）                *
*  args ptr: 包含该线程要读入图片数据的信息（读入多少张，读入图片最终的宽高，      *
*            图片路径等等）                                                        *
*  details： 本函数实际没有做什么，就是深拷贝了args给ptr,然后创建了一个调用        *
*            load_thread()函数的线程并返回线程id                                   *
***********************************************************************************/
...
/***********************************************************************************************************************
*  func: 开辟多个线程读入图片数据，读入数据存储至ptr.d中（主要调用load_in_thread()函数完成）                           *
*  args ptr：包含所有线程要读入图片数据的信息（读入多少张，开几个线程读入，读入图片最终的宽高，图片路径等等）          *
*  details：1) load_threads()是一个指针函数，只是一个返回变量为void*的普通函数，不是函数指针                           *
...
*           流程： 本函数首先会获取要读入图片的张数、要开启线程的个数，而后计算每个线程应该读入的图片张数（尽可能的均  *
*           匀分配），并创建所有的线程，并行读入数据，最后合并每个线程读入的数据至一个大data中，这个data的指针变量与   *
*           ptr的指针变量指向的是统一块内存，因此也就最终将数据读入到ptr.d中（所以其实没有返回值）                     *
***********************************************************************************************************************/
void *load_threads(void *ptr)
{
    ...
    // 就在load_data()中，重新定义了ptr，并为之动态分配了内存，且深拷贝了传给load_data()函数的值args，也就是说在此之后load_data()
	// 函数中的args除了其中的指针变量指着同一块堆内存之外，
    // 二者的非指针变量再无瓜葛，不管之后经过多少个函数，对ptr的非指针变量做了什么改动，比如这里直接free(ptr)，使得非指针变量值为0,
	// 都不会影响load_data()中的args的非指针变量，也就不会影响更为顶层函数中定义的args的非指针变量的值，
    // 比如train_detector()函数中的args，train_detector()对args非指针变量赋的值都不会受影响，保持不变。综其两点，此处直接free(ptr)是安全的。
    // 说明：free(ptr)函数，确定会做的事是使得内存块可以重新分配，且不会影响指针变量ptr本身的值，也就是ptr还是指向那块地址，
    //	    虽然可以使用，但很危险，因为这块内存实际是无效的，
    //      系统已经认为这块内存是可分配的，会毫不考虑的将这块内存分给其他变量，这样，其值随时都可能会被其他变量改变，这种
	//      情况下的ptr指针就是所谓的野指针（所以经常可以看到free之后，置原指针为NULL）。
    ...
    //      关于free()，可以看看：http://blog.sina.com.cn/s/blog_615ec1630102uwle.html，文章最后有一个很有意思的比喻，但意
	//      思好像就和我这里说的有点不一样了（到底是不是编译器搞得鬼呢？？）。
    free(ptr);
...
/***********************************************************************************************
*  func: 开辟线程，读入一次迭代所需的所有图片数据,读入图片的张数为:                            *
*        args.n = net.batch * net.subdivisions * ngpus，读入数据将存入args.d中（虽然args是按值 *
*        传递的，但是args.d是指针变量，函数内改变args.d所指的内容在函数外也是有效的），返回创  *
*        建的读取数据的线程id，以便于外界控制线程的进行                                        *
*  args args：包含要读入图片数据的信息（读入多少张，开几个线程读入，读入图片最终的宽高，图片   *
*             路径等等）                                                                       *
*  details：darknet作者在实现读入图片数据的时候，感觉有点绕来绕去的（也许此中有深意，只是我还  *
*           未明白～），总的流程是：load_data(load_args args)->load_threads(load_args* ptr)->  *
*           load_data_in_thread(load_args args)->load_thread(load_args* ptr),                  *
*           load_thread()函数中会选择具体的读入函数，比如load_data_detection()，要强调的是，所 *
*           有这些函数，都会输入load_args类型参数，或者是其指针，而且很多函数开头就会新建一个  *
*           args，并且深拷贝传入的args，而后返回新建的args，同时可能会随意改动甚至释放传入的   *
*           args，这一整串流程只要记住一点：不管args在之后的函数中怎么变，其中的指针变量所指的 *
*           内存块其实都是一样的（所以不管在哪个函数中，不要随意深层释放args.d的数据，除非是真 *
*           正的用完了，而这里还处于数据读入阶段，所以决不能深层释放args.d）；而非指针变量，每 *
*           个函数都不尽相同，但是没有关系，因为load_data()函数中传入的args是按值传递（不是指  *
*           针）的，不管之后的函数怎么改动args的非指针变量，都不会影响load_data()函数外的值。碎*
*           碎念（TODO）：又是按值传递的哎，虽然上面说道因为按值传递使得更改args非指针变量不会 *
*           影响外界的args，但是传入指针也可以做到啊，反正一进load_data()就深拷贝了args给ptr， *
*           而load_data()也没改动args的非指针变量。                                            *
***********************************************************************************************/
...
data concat_data(data d1, data d2)
{
    data d = {0};
    d.shallow = 1;
	// concat_matrix函数：本文件 合并图片数据至d.X中
...
/*********************************************************************************************
*  func: 合并读入的所有图片数据至一个data中：图片数据不是一起读入的，分了多个线程读入，全部  *
*        存储在d中（d是一个元素类型为data的一维数组）                                        *
*  args d: 读入的所有图片数据，第i个线程读入的部分图片数据存储在d[i]中                       *
*  args n：d的维数，也即数据分了多少块（使用多少个线程读入）                                 *
*  details：虽然返回的out是在函数内新建的，但实际上，out都只是浅拷贝了这些碎块的图片数据，   *
*           也即与这些碎块共享数据，因此，无论是哪个变量，其数据都不能释放（即data.shallow   *
*           只能设置为1,不能深层释放数据的内存）                                             *
*           TODO：为什么总是动不动返回或者传入data等其他结构体实例呢，传指针不是更快么       *
*********************************************************************************************/
...
/********************************************************************************************************************
*  func：从输入d中深拷贝n张图片的数据与标签信息至X与y中：将d.X.vals以及d.y.vals（如有必要）逐行深拷贝至X,y中        *
*  args d: 读入的图片数据，按行存入其中（一张图片对应一行，每一行又按行存储每张图片）                               *
*  args n: 从d中深拷贝n张图片的数据与标签信息到X与y中                                                               *
*  args offset: 相对d首地址的偏移，表示从d中哪张图片开始拷贝（从此往后拷n张图片）                                   *
*  args X: d.X.vals目标存储变量                                                                                     *
*  args y: d.y.vals目标存储变量                                                                                     *
*  details：举例说明，在network.c中的train_network()中，调用了本函数，其中输入n=net.batch（关于net.batch这个参数，  *
*           可以参考network.h中的注释），而d中实际包含了的图片张数为：net.batch*net.subdivision（可以参考detector.c *
*           中的train_detector(),你可能注意到这里没有乘以ngpu，是因为train_network()函数对应处理ngpu=1的情况，如果  *
*           有多个gpu，那么会首先调用train_networks()函数，在其之中会调用get_data_part()将数据平分至每个gpu上，     *
*           而后再调用train_network()，总之最后train_work()的输入d中只包含net.batch*net.subdivision张图片），可知   *
*           本函数只是获取其中一个小batch图片，事实上，从train_network()函数中也可以看出，每次训练一个真实的batch的 *
*           图片（所谓真实的batch，是指网络配置文件中指定的一个batch中所含的图片张数，详细参考network.h中的注释）， *
*           又分了d.X.rows/batch次完成，d.X.rows/batch实际就是net.subdivision的值                                   *
********************************************************************************************************************/
void get_next_batch(data d, int n, int offset, float *X, float *y)
{
    ...
    for(j = 0; j < n; ++j){
        int index = offset + j;
		// memcpy函数：memcpy指的是c和c++使用的内存拷贝函数，memcpy函数的功能是从源src所指的内存地址的起始位置
		// 开始拷贝n个字节到目标dest所指的内存地址的起始位置中。
		// 其原型为：void *memcpy(void *dest, const void *src, size_t n);
		// 从源src所指的内存地址的起始位置开始拷贝n个字节到目标dest所指的内存地址的起始位置中
		// 每次拷贝一行数据，每一行就是一张图片的数据，注意是指针所指内容复制，不是指针复制
        memcpy(X+j*d.X.cols, d.X.vals[index], d.X.cols*sizeof(float));
