...
/*******************************************************************************************
*  func: 深层释放二维数组的堆内存（m.vals是一个float**，即二维数组）                       *
*  args m: 需要释放的矩阵                                                                  *
*  details: 输入虽然是matrix类型数据，但本函数实际是释放其子元素vals的堆内存。             *
*           因为vals是二维数组，之前使用calloc()为每一行动态分配了内存，此外，vals变量本   *
*           身也是一个由calloc(rows, sizeof(float*))动态分配的用来存储每一行指针变量的堆   *
*           内存，因此释放过程首先需要释放每一行的堆内存，最后再直接释放m.vals             *
*******************************************************************************************/
...
matrix make_matrix(int rows, int cols)
{
    ...
    m.rows = rows;
    m.cols = cols;
	// matrix类型的val为二维数组，所以先为行开辟空间，再为每行的列开辟空间
    m.vals = calloc(m.rows, sizeof(float *));
    for(i = 0; i < m.rows; ++i){
		// matrix.vals的值全部初始化为0
        m.vals[i] = calloc(m.cols, sizeof(float));
    }
    return m;
}
