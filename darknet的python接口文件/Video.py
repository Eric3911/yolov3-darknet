...
lib = CDLL("/home/xgh/Model/Darknet-Yolo3/libdarknet.so", RTLD_GLOBAL)
...
def detect(net, meta, im, thresh=.5, hier_thresh=.5, nms=.45):
    ...
    for j in range(num):
        for i in range(meta.classes):
            if dets[j].prob[i] > 0:
                b = dets[j].bbox
                #res.append([meta.names[i], dets[j].prob[i], [b.x+index*299, b.y, b.w, b.h]])
...
if __name__ == "__main__":
    ...
    vid = cv2.VideoCapture('./data/1.mp4')
    #fourcc =cv2.VideoWriter_fourcc(*'XVID')
    #VideoW =cv2.VideoWriter("./out.mp4",fourcc,30.0,(1280,720))
    while True:
        ...
        for i in range(len(boxes)):
            ...
            cv2.putText(arr,str(label),(int(xmin),int(ymin)),fontFace=cv2.FONT_HERSHEY_SIMPLEX,fontScale=0.3,color=(0,255,255),thickness=1)    
